...
const TableScreen = () => {
  ...
  const currentQueueIndex = useAppSelector(
    (state) => state.audio.currentQueueIndex
  );
  const queueLoaded = useAppSelector((state) => state.audio.queueLoaded);
  const waitingForMoodSelection = useAppSelector(
    (state) => state.audio.waitingForMoodSelection
  );
  const showPranSelection = useAppSelector(
    (state) => state.ui.showPranSelection
  );
  ...
  useEffect(() => {
    ...
    const playBackgroundAudio = async () => {
      try {
        ...
        await audio.play();
        console.log(
          "✅ Background audio (bg1) playing in loop at volume:",
          audio.volume
        );
      } catch (error) {
    ...
    const handleError = (e) => {
      ...
      if (
        ...
        console.warn(
          "Background audio (bg1): Ignoring error with empty/invalid src"
        );
        return;
      }
      ...
      if (audio.error?.code === 4) {
        ...
        console.error(
          "   Check CloudFront CORS configuration (see CLOUDFRONT_CORS_SETUP.md)"
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const playBg2Audio = async () => {
      try {
        ...
        await audio.play();
        console.log(
          "✅ Background audio (bg2) playing in loop at volume:",
          audio.volume
        );
      } catch (error) {
    ...
    const handleError = (e) => {
      ...
      if (
        ...
        console.warn(
          "Background audio (bg2): Ignoring error with empty/invalid src"
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    if (
      ...
      sessionStorage.getItem("audio_unlocked")
    ) {
      console.log(
        "Clearing audio_unlocked from sessionStorage to force fresh permission check"
      );
  ...
  useEffect(() => {
    ...
    socket.on("phase_changed", ({ phase }) => {
      ...
      if (
        phase !== "CATEGORY_FLOW" &&
        phase !== "PRAN_SELECTION" &&
        phase !== "ENDING" &&
        phase !== "MOOD_SELECTION"
      ) {
      ...
      if (
        phase === "ENDING" &&
        audioQueue.length > 0 &&
        currentQueueIndex < audioQueue.length - 1
      ) {
        console.log(
          "ENDING phase: Continuing audio queue from index",
          currentQueueIndex
        );
    ...
    socket.on("audio_play", ({ audioPath, audioId }) => {
      ...
      if (currentPhase === "COMMON_FLOW" && audioQueue.length > 0) {
        ...
        const currentAudioInQueue = audioQueue.find(
          (a) =>
            a._id === audioId ||
            a.filePath === audioPath ||
            (a.sequence === 8 &&
              (audioPath.includes("Choosing-Emotion") ||
                audioPath.includes("choosing-emotion")))
        );

        if (currentAudioInQueue && currentAudioInQueue.sequence === 8) {
          console.log(
            "Audio 8 (Choosing-Emotion) detected - showing mood selection as fallback"
          );
    ...
    socket.on("pran_selected", async ({ pranId }) => {
      ...
      try {
        if (session?.category) {
          const pransResponse = await api.get(
            `/api/pran?category=${session.category}`
          );
          ...
          const pranData = pransResponse.data.find(
            (p) => p.id === pranId || p._id === pranId
          );
          if (pranData) {
      ...
      if (currentQueueIndex < audioQueue.length - 1) {
        ...
        console.log(
          "Pran selected - Continuing to next audio:",
          nextAudio.fileName
        );
    ...
    socket.on("cue_trigger", ({ cuePoint, data }) => {
      ...
      if (cuePoint === "PRAN_SELECTION") {
        ...
        console.log(
          "MOOD_SELECTION cue triggered - showing mood selection immediately"
        );
        // Set waiting flag - we're waiting for user to select mood
        dispatch(setWaitingForMoodSelection(true));
    ...
    const loadAudioQueue = async () => {
      if (currentPhase === "COMMON_FLOW" && !queueLoaded) {
        ...
        const bg1Audio = response.data.find((audio) => {
          ...
          return (
            fileName === "bg1.mp3" ||
            (fileName.includes("bg1") && fileName.endsWith(".mp3"))
          );
        });
        ...
        if (bg1Audio) {
          ...
          if (!pran) {
            // Validate URL before setting
            const audioUrl =
              bg1Audio.filePath.startsWith("http://") ||
              bg1Audio.filePath.startsWith("https://")
                ? bg1Audio.filePath
                : `${
                    process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001"
                  }${bg1Audio.filePath}`;
          ...
          console.warn(
            "bg1.mp3 not found in COMMON audio files. Available files:",
            response.data.map((a) => a.fileName)
          );
        ...
        if (sorted.length > 0 && socket) {
          console.log(
            `Playing first audio: ${sorted[0].fileName} (sequence: ${sorted[0].sequence}, cuePoint: ${sorted[0].cuePoint})`
          );
          socket.emit("audio_play", {
        ...
        currentPhase === "CATEGORY_FLOW" &&
        session?.category &&
        !queueLoaded
      ) {
        console.log(
          `Loading CATEGORY_FLOW audio queue for ${session.category}...`
        );
        const response = await api.get(
          `/api/audio?category=${session.category}`
        );
        const sorted = response.data.sort((a, b) => a.sequence - b.sequence);
        dispatch(setAudioQueue(sorted));
        dispatch(setCurrentQueueIndex(0));
  ...
  const handleAudioEnd = () => {
    console.log(
      `Audio ended. Current index: ${currentQueueIndex}, Queue length: ${audioQueue.length}, Phase: ${currentPhase}, Waiting for mood: ${waitingForMoodSelection}`
    );
    ...
    if (session?.category === "POSITIVE") {
      if (currentAudioInQueue && currentAudioInQueue.sequence === 2) {
        console.log(
          "Positive Flow: Audio 2 ended - NOT auto-playing audio 3. Images will show for 10 seconds, then audio 3 will play."
        );
        dispatch(clearCurrentAudio());
    ...
    if (session?.category === "NEUTRAL") {
      if (currentAudioInQueue && currentAudioInQueue.sequence === 1) {
        console.log(
          "Neutral Flow: Audio 1 ended - NOT auto-playing audio 2. Images will show for 10 seconds, then audio 2 will play."
        );
        dispatch(clearCurrentAudio());
    ...
    if (session?.category === "NEGATIVE") {
      if (currentAudioInQueue && currentAudioInQueue.sequence === 1) {
        console.log(
          "Negative Flow: Audio 1 ended - NOT auto-playing audio 2. Images will show for 10 seconds, then audio 2 will play."
        );
        dispatch(clearCurrentAudio());
        // Don't increment queue index - mirror.js will handle playing audio 2 after images
        return; // Don't continue to next audio
    ...
    if (session?.category === "NEGATIVE") {
      if (currentAudioInQueue && currentAudioInQueue.sequence === 2) {
        console.log(
          "Negative Flow: Audio 2 ended - NOT auto-playing audio 3. Images will show for 10 seconds, then audio 3 will play."
        );
        dispatch(clearCurrentAudio());
        // Don't increment queue index - mirror.js will handle playing audio 3 after images
        return; // Don't continue to next audio
    ...
    if (
      currentAudioInQueue &&
      currentAudioInQueue.cuePoint === "PRAN_SELECTION"
    ) {
      // If pran hasn't been selected yet, don't play next audio
      if (!pran) {
        console.log(
          "PRAN_SELECTION audio ended - Waiting for user to select pran before playing next audio"
        );
        dispatch(clearCurrentAudio());
        return; // Don't continue to next audio until pran is selected
      } else {
        console.log(
          "PRAN_SELECTION audio ended - Pran already selected, continuing to next audio"
        );
      }
    }
    ...
    if (currentPhase === "PRAN_SELECTION" && !pran) {
      console.log(
        "In PRAN_SELECTION phase but pran not selected - waiting for pran selection"
      );
      dispatch(clearCurrentAudio());
    ...
    if (currentQueueIndex < audioQueue.length - 1) {
      ...
      if (currentPhase === "COMMON_FLOW" && socket) {
        ...
        console.log(
          "Common flow finished - mood selection should already be visible"
        );
      }
    ...
    <div className="min-h-screen bg-black">
      ...
      {!audioPermissionGranted && (
        <AudioPermissionPrompt
          onPermissionGranted={() => {
            console.log(
              "✅ Audio permission granted - enabling audio playback"
            );
      ...
      {audioPermissionGranted && backgroundAudioPath && (
        <audio
          ...
          src={
            backgroundAudioPath.startsWith("http://") ||
            backgroundAudioPath.startsWith("https://")
              ? backgroundAudioPath
              : `${
                  process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001"
                }${encodeURI(backgroundAudioPath)}`
          }
          loop
          preload="auto"
          style={{ display: "none" }}
          ...
          onError={(e) => {
            ...
            console.error(
              "Audio error details:",
              backgroundAudioRef.current?.error
            );
          }}
          onLoadedMetadata={() => {
      ...
      {audioPermissionGranted && bg2AudioPath && (
        <audio
          ref={bg2AudioRef}
          src={
            bg2AudioPath.startsWith("http://") ||
            bg2AudioPath.startsWith("https://")
              ? bg2AudioPath
              : `${
                  process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001"
                }${encodeURI(bg2AudioPath)}`
          }
          loop
          preload="auto"
          style={{ display: "none" }}
